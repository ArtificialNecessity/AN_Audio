...
/// <summary>
/// IMidiInput on WinMM (SPEC-30 §5). Owns the policy, the poll/worker thread, the open-port slot table, and
/// identity orchestration. Driver-thread entry points are DeliverFromDriver/QueueSysExFromDriver; everything
/// else runs on the worker thread or the caller's control thread.
/// </summary>
internal sealed class WinMm_MidiInput : IMidiInput
{
    private const int MaxPortSlots = 256;   // MidiInput_PortIndex is a byte

    private readonly MidiInput_Options _options;
    private readonly object _controlLock = new();
    private readonly WinMm_MidiInPort?[] _slots = new WinMm_MidiInPort?[MaxPortSlots];
    ...
    private readonly ConcurrentQueue<MidiInput_PortIndex> _driverClosedPorts = new();
    ...
    public WinMm_MidiInput(MidiInput_Options options)
    {
        options.Validate();
        _options = options;
    ...
    private void StartCore(MidiInput_Callback? raw)
    {
        ...
        lock (_controlLock)
        {
            ...
            _running = true;
            _worker = new Thread(WorkerLoop) { Name = "AN.Audio.Midi WinMM worker", IsBackground = true };
            _worker.Start();
    ...
    // ── driver-thread entry points (hot path) ──────────────────────────────────────────────────
    ...
    internal void NoteDriverLag() => Ring.NoteDriverLag();
    ...
    {
        _driverClosedPorts.Enqueue(port);
        _wake.Set();
    }
    ...
    private void Rescan()
    {
        List<WinMm_MidiInDeviceEntry> present;
        try { present = WinMm_MidiInEnumerator.EnumerateInputs(); }
        catch { return; }
        ...
        foreach (var entry in present)
        {
            ...
            var port = new WinMm_MidiInPort(this, _options, new MidiInput_PortIndex((byte)slot), entry);
            var r = port.Open();
            if (r == WinMm_Result.NoError)
            {
                _slots[slot] = port;
            ...
            else
            {
                ...
                var reason = r == WinMm_Result.Allocated ? MidiInput_LostReason.InUseByAnotherApplication : MidiInput_LostReason.DriverError;
    ...
    private void DrainDriverClosedPorts()
    {
        while (_driverClosedPorts.TryDequeue(out var port))
    ...
    private void DrainIdentityRequests()
    {
        ...
        while (_identityRequests.TryDequeue(out var portIndex))
        {
            ...
            if (port is null || !port.IsOpen || !port.Entry.HasPairedOutput) continue;
            var outIndex = WinMm_MidiInEnumerator.FindPairedOutputIndex(port.Entry.Name);
            if (outIndex is null) continue;

            WinMm_MidiOutLongSender.TrySendLongMessage(outIndex.Value, request, _options.IdentityReplyTimeoutMs);   // D9: silent on failure
        }
    }
