...
| D27 | **Packaging — supersedes D1/D21** | **Two independent packages, one per project, FluidUI-style**: `AN.Audio.csproj` → `ArtificialNecessity.Audio`, `AN.Audio.Midi.csproj` → `ArtificialNecessity.Audio.Midi`. Each csproj is `IsPackable=true` with its own metadata; README/LICENSE are packed into both. **No `AN.Audio.Package` project, no cross-project pack hooks.** Scripts `dotnet pack` the solution; `AN.Audio.Build.props`'s `DeployToLocalNuGet` copies each nupkg to the feed. Verified 2026-09-08: `cmd\publish-local.cmd` deploys both, each with `lib/net8.0|net9.0|net10.0/<own>.dll`. | User 2026-09-08: the umbrella project was cruft ("we do not want any AN.Audio.Package"); after D24 the libraries are independent, so separate packages is the honest shape. It also fixed a real bug: the umbrella's pack hook produced a 10 KB nupkg with **no DLLs** under `dotnet pack --no-build`. |
