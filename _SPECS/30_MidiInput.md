...
- [ ] Sprint 1 — Windows/WinMM input: enumerate, open-all, short messages, SysEx Identity Request/Reply, polling hot-plug, ring + raw callback, tests
...
| D11 | Device identity | Two identities per port. **Instance key** `MidiInput_DeviceKey` = `MIDIINCAPS2W.NameGuid`/`ProductGuid` when non-zero, else `(name, wMid, wPid, ordinal-among-same-name)`; WinMM index is never exposed. **Type id** `MidiInput_DeviceTypeId` (GUID) = deterministic hash of the Identity Reply `(manufacturer, family, member)`; when no reply arrives, a hash of `(szPname, wMid, wPid)` flagged `IsUnknownType`. Consumers address gear by `TypeId` (per-device-type settings, mappings) and use `Key` only to tell two units apart. **Known limitation:** with GUID-less drivers and two identical units, unplugging the first renumbers the second's ordinal → it is reported `DeviceLost` + `DeviceOpened` under a new `Key`; its `TypeId` is unchanged, so type-keyed consumers are unaffected. Per-unit serials (vendor SysEx / MIDI-CI) are future work. | Indices renumber on unplug. User intent 2026-09-07: stable per-device-TYPE ids are the goal of the SysEx identity work; two-identical-units is the accepted edge case until vendors expose serials. |
...
| D23 | Growable ring | `MidiInput_MessageRing` is an **encapsulated, growable** SPSC queue: `RingInitialCapacity` (default 1024) up to `RingMaxCapacity` (default 16384 = 256 KB of 16-byte messages; `Max == Initial` means fixed). **No power-of-two requirement** (modulo instead of mask; irrelevant cost). Internally a linked list of fixed-size segments: when the tail segment is full and `CurrentCapacity < MaxCapacity`, the producer (driver thread) allocates one more segment and links it with a single `Volatile.Write` — nothing is copied, the consumer simply follows `Next` after draining. Drained segments are recycled to the tail, never freed, so once the queue has grown to its working size the steady state is allocation-free. At `MaxCapacity` the D5 drop-newest rule applies. Consumers never see storage; only `TryDequeue`/`DequeueAll` and the counters. | User preference 2026-09-07: a bounded number of allocations while converging to stability is acceptable; a per-loop allocation is not. Segments (not realloc+copy) because the consumer may be mid-dequeue when the producer grows. Optional future refinement: a background thread pre-allocates one spare segment so the driver thread never allocates. |
...
{
    public int  CurrentCapacity { get; }                       // D23 — starts at RingInitialCapacity, grows up to RingMaxCapacity, never shrinks
...
public interface IMidiInput : IDisposable
{
    ...
    IReadOnlyList<MidiInput_DeviceInfo> OpenPorts { get; }     // index == MidiInput_PortIndex
...
`Internal/`: `MidiInput_MessageRing` (D23: segmented SPSC — `Volatile` head/tail per segment, `Volatile` `Next` link; producer allocates a new segment when the tail is full and below `RingMaxCapacity`; drained segments are recycled, never freed), `Midi_SysExReassembler`, `Midi_IdentityReplyParser`.
...
- Identity out-port lifecycle (D9: open → send → close on `MOM_DONE`) vs. holding the out-port open for the life of the in-port: validate with real devices in Sprint 1 (`SimpleMidiTest`); revisit when Sprint 3 needs a persistent `IMidiOutput` handle anyway.
