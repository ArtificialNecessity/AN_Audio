...
- **Design spec (authoritative):** `50_Audio_Formats.md` (Draft v2, D1–D16). This file is the checklist; when they disagree, the design spec wins and this file gets fixed.
...
- **Ground truth already on disk:** `_EXTERNAL_APIS/SimpleFlac_FlacDecoder.md`, `_EXTERNAL_APIS/NLayer_MpegFile.md`, `src/AN.Audio.Formats/Flac/Flac_ReferenceDecoder.cs` (subsumed SimpleFlac @ `dc149aa`, body untouched, provenance header + MIT notice) and `Flac/LICENSE-SimpleFlac.txt`. Nothing under `src/AN.Audio.Formats/` is in git yet.
...
- [x] `AudioSampleConvert.cs` — `Convert(ReadOnlySpan<byte> src, AudioFormat from, Span<byte> dst, AudioFormat to)` for all 6×6 pairs (same rate/channels; throw otherwise); move the existing conversion out of `AN.Audio/Internal/` with `git mv` where the code maps 1:1, delete the remainder
...
- [ ] `MusicStudio.Build.props` `ANAudioVersion` → the version above; add `<PackageReference Include="ArtificialNecessity.Audio.Formats" />`
...
- **Q1** `ffmpeg`/`flac`/`lame` availability for fixture rendering (check `Get-Command ffmpeg` first; else pick small permissively-licensed files and record provenance)
- **Q2** NLayer 3.0.0 package TFMs/dependencies — verify before pinning
- **Q3** Per-format metadata on `AudioDecoder_Pcm`? Current answer: no — use the per-format tier