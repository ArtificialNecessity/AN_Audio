...
- [x] 4c: RF64/BW64 (`ds64`: RIFF size, `data` size, sample count, table → `Wav_DataSize64Chunk`) and Wave64 (GUID ids, u64 sizes incl. header, 8-byte alignment) in `Wav_Decoder` via `Wav_ContainerLayout` + `TryReadChunkHeader`/`PadBytesFor`; `Sniff` recognises the `riff…wave` GUID pair (peek 40) and `BW64`; `Wav_TestWriter.BuildRf64/BuildW64` emit the same chunk list in each layout; `WavRf64W64Tests` (13): ds64 data size, table-resolved chunks, BW64, oversize declaration → clamp + `EndedEarly`, real 32-bit sizes inside RF64, Wave64 bit-exact + 8-byte padding + seek + truncation + bad GUID → `FormatException`, RIFF unchanged; all seekable + forward-only
